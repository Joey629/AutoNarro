...
# [優化] 訓練函數增加了 pos_weight 和 early stopping 機制
def train(model, train_dataloader, val_dataloader, epochs, lr, device, pos_weight, patience=3):
    print("\n--- 开始训练【优化版】线索增强式问答模型 ---")
    model.to(device)
    
    # [優化] 使用传入的 pos_weight 初始化损失函数，以解决类别不平衡问题
    criterion = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
    print(f"BCEWithLogitsLoss 已启用 pos_weight: {pos_weight.item():.2f}")
    ...
    for epoch in range(epochs):
        ...
        if f1 > best_f1:
            best_f1 = f1
            torch.save(model.state_dict(), "clue_augmented_model_best_weights.pth")
            print("当前为最佳模型，权重已保存。")
            # [優化] 重置早停计数器
            patience_counter = 0
        else:
...
def evaluate(model, test_dataloader, device):
    ...
    num_tasks = 15
    # Important: Ensure the dataloader is NOT shuffling for evaluation, so reshaping is correct
    num_samples = len(all_labels_flat)
    per_task_f1_scores = None
    ...
    if num_samples > 0 and num_samples % num_tasks == 0:
        ...
        print(f"警告: 无法计算各任务分数。样本总数 {num_samples} 不能被任务数 {num_tasks} 整除。")

    return overall_f1, overall_accuracy, per_task_f1_scores


def create_expanded_dataset(df, task_names, question_templates):
    ...
    for index, row in df.iterrows():
        ...
        if story_type in question_templates:
            for task_name in task_names:
                ...
                new_text = f"问题：{question} 叙事：{original_text} 已知线索：{ist_words}"
...
if __name__ == "__main__":
    ...
    print("开始划分原始数据集...")
    # 保持 train_df, test_df 的划分，以便和上次结果进行公平比较
    train_df, test_df = train_test_split(df, test_size=0.1, random_state=42)
    
    TASK_NAMES = [f'A{i}' for i in range(2, 17)]
    
    print("开始为训练集生成问答式数据...")
    train_expanded_texts, train_expanded_labels = create_expanded_dataset(train_df, TASK_NAMES, QUESTION_TEMPLATES)
    
    print("开始为测试集生成问答式数据...")
    test_expanded_texts, test_expanded_labels = create_expanded_dataset(test_df, TASK_NAMES, QUESTION_TEMPLATES)
    
    print(f"数据集生成完毕！训练集 {len(train_expanded_texts)} 条, 测试集 {len(test_expanded_texts)} 条。")

    print("开始文本分词...")
    tokenizer = AutoTokenizer.from_pretrained('hfl/chinese-roberta-wwm-ext')
    train_encodings = tokenizer(train_expanded_texts, padding=True, truncation=True, max_length=512, return_tensors='pt')
    test_encodings = tokenizer(test_expanded_texts, padding=True, truncation=True, max_length=512, return_tensors='pt')

    train_labels = torch.tensor(train_expanded_labels, dtype=torch.float32).reshape(-1, 1)
    test_labels = torch.tensor(test_expanded_labels, dtype=torch.float32).reshape(-1, 1)
    ...
    # 评估时使用 SequentialSampler，确保数据顺序不变
    ...
    # [優化] 1. 计算 pos_weight
    train_labels_np = np.array(train_expanded_labels)
    num_pos = np.sum(train_labels_np == 1)
    num_neg = np.sum(train_labels_np == 0)
    # 加上一个极小值防止分母为0
    pos_weight_value = num_neg / (num_pos + 1e-9)
    # 将 pos_weight 转换成 tensor 并移动到正确的设备
    ...
    # [優化] 2. 调整超参数 epochs 和 lr，并传入 pos_weight
    ...
    final_model.load_state_dict(torch.load("clue_augmented_model_best_weights.pth"))
